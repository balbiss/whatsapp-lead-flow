...
import { QrCode, Wifi, WifiOff, RefreshCw, Smartphone, Info, Clock, MessageSquare } from "lucide-react";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-foreground">
                  {status === "online" ? "Ativo" : status === "connecting" ? "Conectando" : "Desconectado"}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-foreground">Wuzapi v2</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-border">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> Uptime</span>
                <span className="font-medium text-foreground">{status === "online" ? "2h 34min" : "—"}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Mensagens hoje</span>
                <span className="font-medium text-foreground">{status === "online" ? "127" : "0"}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Contatos ativos</span>
                <span className="font-medium text-foreground">{status === "online" ? "34" : "0"}</span>
              </div>
            </CardContent>
          </Card>
        </div>
